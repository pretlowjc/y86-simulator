...
bool DecodeStage::doClockLow(PipeRegArray *pipeRegs)
{
	...
	setSrcA(ereg, icode, d_srcA, rA, RegisterFile::rsp);
	setSrcB(ereg, icode, d_srcB, rB, RegisterFile::rsp);
	setDstE(ereg, icode, dstE, rB, RegisterFile::rsp);
	setDstM(ereg, icode, dstM, rA);
...
uint64_t DecodeStage::setSrcB(PipeReg *ereg, uint64_t D_icode, uint64_t d_srcB, uint64_t D_rB, uint64_t RSP){
	if(D_icode == Instruction::IOPQ || Instruction::IRMMOVQ || Instruction::IMRMOVQ) return d_srcB = D_rB;
	if(D_icode == Instruction::IPUSHQ || Instruction::IPOPQ || Instruction::ICALL || Instruction::IRET) return d_srcB = RSP;
	return RegisterFile::RNONE;

}

uint64_t DecodeStage::setDstE(PipeReg *ereg, uint64_t D_icode, uint64_t d_dstE, uint64_t D_rB, uint64_t RSP){
	if(D_icode == Instruction::IRMMOVQ || Instruction::IIRMOVQ || Instruction::IOPQ) return d_dstE = D_rB;
	if(D_icode == Instruction::IPUSHQ || Instruction:: IPOPQ || Instruction::ICALL || Instruction::IRET) return d_dstE = RSP;
	return RegisterFile::RNONE;
}

uint64_t DecodeStage::setDstM(PipeReg *ereg, uint64_t D_icode, uint64_t d_dstM, uint64_t D_rA){
	if(D_icode == Instruction::IMRMOVQ || Instruction::IPOPQ) return d_dstM = D_rA;
	return RegisterFile::RNONE;
}
