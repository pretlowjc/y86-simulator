...
{
   //make sure the RegisterFile singleton has been created
   assert(this->rf != NULL);
   
   //call getInstance again to make sure it returns the same 
   //instance.  If this fails, your RegisterFile singleton
   //hasn't been set up properly.
   assert(this->rf == RegisterFile::getInstance());
...
void RegisterFileTester::doClearRegisterTests()
{
   ...
   bool error;
   uint64_t value;
   for (regNumber = 0; regNumber < RegisterFile::size; regNumber++)
   {
